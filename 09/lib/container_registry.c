...
void init_container_registry(void) {
    ...
    extern Container create_linkedlist_container(int allocation_policy);
    
    // Simple wrapper functions
    Container create_linkedlist_seq() { return create_linkedlist_container(0); }
    Container create_linkedlist_rand() { return create_linkedlist_container(1); }
    
    register_container("array", "Dynamic array container", create_array_container);
    register_container("linkedlist_seq", "Linked list with sequential allocation", create_linkedlist_seq);
    register_container("linkedlist_rand", "Linked list with random allocation", create_linkedlist_rand);
}
...
void cleanup_container_registry(void) {
    ...
    while (current) {
        ...
        if (current->description) free((void*)current->description);
        free(current);
        current = next;
    }
    registry_head = NULL;
